	...
	*/
	...
	jQuery(document).ready(function($) {
		...
		jQuery.get(ltvars.ajaxurl, data, function(response) {
			...
			jQuery.each(response.data, function(name, val) {
...
//console.log(jQuery('#' + name).filter('input[type=text]').attr('ans'));
	...
	// input field lose focus - maybe this is too much
	jQuery(window).on("beforeunload", function() { 
	    save_user_data();
	})
	
	
	// END TRIGGERS TO SAVE DATA -----------------------
    ...
    jQuery('.text').keyup(function(e){
     ...
     if(typeof jQuery(this).attr('ans') !== 'undefined') {
      ...
      var answer = jQuery(this).attr('ans').toLowerCase();
    ...
    ****************************************/
    ...
    jQuery('.dropdown').change(function(e){
      ...
      if(answer=='correct') {
	//selectedoption.parent().css({'background':'green','font-weight':'bolder','color':'white'});
	selectedoption.parent().addClass("correct");
      }
    });
